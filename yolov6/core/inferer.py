...
class Inferer:
    ...
    def infer(self, conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, save_img, hide_labels, hide_conf, view_img=True):
        ...
        for img_src, img_path, vid_cap in tqdm(self.files):
            ...
            csvfile = open('E:\\IISc\\Object_detection\\YOLOv6\\YOLOv6-main\\tools\\runs\\inference\\exp\\yolo6_y.csv',
                           'w', encoding='utf-8')
            ...
            txt_path = 'E:\\IISc\\Object_detection\\YOLOv6\\YOLOv6-main\\tools\\runs\\inference\\exp\\here\\yolo6_y.txt'
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    #     xywh = (self.box_convert(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                        xywh = self.box_convert(torch.tensor(xyxy).view(1, 4)).view(-1).tolist()
                        # print("xyxy", xywh)
                        line = (cls, *xywh, conf)
                        new_line = ('%g ' * len(line)).rstrip() % line + '\n'

                        # print("line", new_line)
                        box_points = new_line.split(' ')[1:5]
                        box = [int(y) for y in [float(x) for x in box_points]]
                        label = labels_to_names[int(new_line.split(' ')[0])]
                        score = float(new_line.split(' ')[5])

                        csv_line = [cnt, box, label, score]
                        csvfile_writer.writerow(csv_line)
                        cnt += 1
                        # csvfile_writer.writerow(new_line)
                        print("class", labels_to_names[int(new_line.split(' ')[0])])
                        print("box", [int(y) for y in [float(x) for x in box_points]])
                        print("conf", new_line.split(' ')[5])
