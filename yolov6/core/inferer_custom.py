...
class Inferer:
    def __init__(self, source, weights, device, yaml, img_size, half):
        ...
        self.device = torch.device(f'cuda:{0}' if cuda else 'cpu')
    ...
    def infer(self, conf_thres, iou_thres, classes, agnostic_nms, max_det, save_dir, save_txt, save_img, hide_labels, hide_conf, view_img=True):
        ...
        for img_src, img_path, vid_cap in self.files:
            ...
            csvfile = open('E:\\IISc\\Object_detection\\YOLOv6\\YOLOv6-main\\Evaluation\\output_files\\yolo6_y.csv', 'w', encoding='utf-8')
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    #     xywh = (self.box_convert(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
                        ...
                        # print("int xyxy", [int(i) for i in xyxy])

                        label = labels_to_names[int(new_line.split(' ')[0])]
                        score = float(new_line.split(' ')[5])

                        csv_line = [cnt, [int(i) for i in xyxy], label, score]
